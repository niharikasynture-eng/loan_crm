...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!blandKey) {
      console.error('CRITICAL: BLAND_API_KEY is missing from .env.local');
      return NextResponse.json({ message: 'AI Voice service not configured' }, { status: 500 });
    }

    console.log('In Promo Route (V3) - Lead ID:', leadId);
    await connectDB();
    ...
    if (!lead.phone) {
      return NextResponse.json({ message: 'Lead has no phone number' }, { status: 400 });
    }
    // Trigger AI Call
    ...
    console.error('PROMO ROUTE (V2) CRITICAL ERROR:', err);
